...
function hydrate(s) {
  ...
  return {
    ...
    bankTxns: Array.isArray(s.bankTxns) ? s.bankTxns : [],
    bandgang: {
...
const BANK_LABELS = { chime: "Chime", cashapp: "Cash App", navyfed: "Navy Fed" };
const BANK_COLORS = { chime: "#1ec677", cashapp: "#00c532", navyfed: "#5b8dd9" };
...
function parseCSV(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let c = 0; c < line.length; c++) {
      ...
      if (ch === '"' && inQuote) {
        if (line[c + 1] === '"') { field += '"'; c++; } // escaped quote
        else inQuote = false;
        continue;
      }
...
// ---- Parse amount, strip $ commas quotes ----
function parseAmt(s) {
  if (s == null) return 0;
...
function parseChimeCSV(text) {
  const rows = parseCSV(text);
  if (rows.length < 2) return [];
  const hdr = rows[0].map(h => h.toLowerCase().trim());
  const dateIdx = hdr.findIndex(h => h.includes("date"));
  const descIdx = hdr.findIndex(h => h.includes("description") || h.includes("name") || h.includes("memo"));
  const amtIdx  = hdr.findIndex(h => h === "amount");
...
// ---- Cash App CSV parser ----
// Headers: Date, Transaction Type, Currency, Amount, Fee, Net Amount, Asset, Asset Type, Transaction ID, Note
// Amount negative = sent, positive = received
function parseCashAppCSV(text) {
  ...
  const dateIdx   = hdr.findIndex(h => h.includes("date"));
  const amtIdx    = hdr.findIndex(h => h === "amount");
  const noteIdx   = hdr.findIndex(h => h.includes("note") || h.includes("description"));
  const typeIdx   = hdr.findIndex(h => h.includes("transaction type") || h.includes("type"));
  const netIdx    = hdr.findIndex(h => h.includes("net amount"));
  const useAmt    = netIdx >= 0 ? netIdx : amtIdx;
  if (dateIdx < 0 || useAmt < 0) return [];
  return rows.slice(1).map(row => {
    const amt = parseAmt(row[useAmt]);
    if (amt === 0) return null;
    const isCredit = amt > 0;
    const desc = row[noteIdx] || row[typeIdx] || "Transaction";
    return {
      id: uid(),
      source: "cashapp",
      date: normalizeDate(row[dateIdx]),
...
// ---- Navy Federal CSV parser ----
// Headers: Account Number, Transaction Date, Transaction Type, Transaction Description, Debit Amount, Credit Amount, Balance
// Debit = expense (positive number), Credit = income (positive number)
function parseNavyFedCSV(text) {
  const rows = parseCSV(text);
  if (rows.length < 2) return [];
  const hdr = rows[0].map(h => h.toLowerCase().trim());
  const dateIdx   = hdr.findIndex(h => h.includes("date"));
  const descIdx   = hdr.findIndex(h => h.includes("description") || h.includes("memo") || h.includes("transaction description"));
  const debitIdx  = hdr.findIndex(h => h.includes("debit"));
  const creditIdx = hdr.findIndex(h => h.includes("credit") && !h.includes("account"));
  if (dateIdx < 0) return [];
  return rows.slice(1).map(row => {
...
// ---- Import trigger ----
function importBankCSV(source) {
  const el = document.getElementById("csv-" + source);
  if (el) { el.value = ""; el.click(); }
}

// ---- Handle file selected ----
function handleBankCSV(source, input) {
  ...
  reader.onload = e => {
    ...
    let parsed = [];
    if (source === "chime")   parsed = parseChimeCSV(text);
    if (source === "cashapp") parsed = parseCashAppCSV(text);
    if (source === "navyfed") parsed = parseNavyFedCSV(text);

    if (!parsed.length) {
      showToast("No transactions found. Check the file format guide below.");
      return;
    }

    // Deduplicate: skip rows that look identical (same source+date+description+amount)
    ...
    const newTxns = parsed.filter(t =>
      !existing.has(`${t.date}|${t.description}|${t.amount}`)
    );

    state.bankTxns.push(...newTxns);
    // Keep sorted newest first
    state.bankTxns.sort((a, b) => (b.date || "").localeCompare(a.date || ""));
    saveState();

    const label = BANK_LABELS[source] || source;
    if (newTxns.length === 0) {
      showToast(`${label}: All ${parsed.length} transactions already imported.`);
    } else {
      showToast(`${label}: Imported ${newTxns.length} new transaction${newTxns.length !== 1 ? "s" : ""}!`);
    }

    // Switch to that bank's tab automatically
    bankFeedTab = source;
    renderAccounts();
    // Highlight the correct tab button
    document.querySelectorAll(".feed-tab").forEach(b => {
      b.classList.toggle("active", b.textContent.trim().toLowerCase().replace(" ","") ===
        (source === "cashapp" ? "cashapp" : source === "navyfed" ? "navyfed" : source));
    });
  };
...
// ---- Switch bank feed sub-tab ----
function setBankFeedTab(source, btn) {
  bankFeedTab = source;
...
function deleteBankTxn(id) {
  state.bankTxns = state.bankTxns.filter(t => t.id !== id);
  saveState();
  renderBankFeed();
  updateBankCounts();
}

// ---- Clear all txns for one source ----
function clearBankSource(source) {
  const label = BANK_LABELS[source] || source;
  if (!confirm(`Clear all ${label} transactions?`)) return;
  state.bankTxns = state.bankTxns.filter(t => t.source !== source);
  saveState();
  renderBankFeed();
  updateBankCounts();
  showToast(`${label} transactions cleared.`);
}

// ---- Clear all ----
function clearAllBankTxns() {
  if (!state.bankTxns.length) { showToast("No bank transactions to clear."); return; }
  if (!confirm("Clear ALL imported bank transactions?")) return;
  state.bankTxns = [];
  saveState();
  renderBankFeed();
  updateBankCounts();
  showToast("All bank transactions cleared.");
}

// ---- Update count badges ----
function updateBankCounts() {
  ["chime","cashapp","navyfed"].forEach(src => {
    const el = document.getElementById("count-" + src);
    if (!el) return;
    const n = state.bankTxns.filter(t => t.source === src).length;
    el.textContent = n + " txn" + (n !== 1 ? "s" : "");
  });
}

// ---- Render the bank feed list ----
function renderBankFeed() {
  const el = document.getElementById("bankFeedList");
  const summaryEl = document.getElementById("feedSummaryRow");
  if (!el) return;

  const source = bankFeedTab;
  const txns = source === "all"
    ? state.bankTxns.slice()
    : state.bankTxns.filter(t => t.source === source);
  ...
  if (summaryEl) {
    summaryEl.innerHTML = txns.length ? `
      <div class="feed-summary-stat">
        <div class="feed-summary-label">Deposits</div>
        <div class="feed-summary-val deposit">${fmt(deposits)}</div>
      </div>
      <div class="feed-summary-stat">
        <div class="feed-summary-label">Expenses</div>
        <div class="feed-summary-val expense">${fmt(expenses)}</div>
      </div>
      <div class="feed-summary-stat">
        <div class="feed-summary-label">Net</div>
        <div class="feed-summary-val ${deposits - expenses >= 0 ? "deposit" : "expense"}">${fmt(deposits - expenses)}</div>
      </div>` : "";
  }

  if (!txns.length) {
    const label = source === "all" ? "any account" : BANK_LABELS[source];
    el.innerHTML = `<div class="empty">No transactions from ${label}. Tap the import button above to load your CSV.</div>`;
    return;
  }
  ...
  el.innerHTML = dates.map(date => {
    ...
    const rows = dayTxns.map(t => {
      ...
      const srcLabel = BANK_LABELS[t.source] || t.source;
      return `
        <div class="bank-txn-row ${t.source}">
          <div class="bank-txn-info">
            <div class="bank-txn-desc">${escapeHtml(t.description)}</div>
            <div class="bank-txn-meta">
              <span class="bank-txn-badge ${t.source}">${srcLabel}</span>
              <span class="bank-txn-type ${isDeposit ? "deposit" : "expense"}">${isDeposit ? "DEPOSIT" : "EXPENSE"}</span>
            </div>
          </div>
          <div class="bank-txn-right">
            <div class="bank-txn-amt ${isDeposit ? "deposit" : "expense"}">${isDeposit ? "+" : "-"}${fmt(t.amount)}</div>
            <button class="bank-txn-del" onclick="deleteBankTxn('${t.id}')" title="Remove">&#10005;</button>
          </div>
        </div>`;
    }).join("");
    ...
    return `
      <div class="feed-date-group">
        <div class="feed-date-label">${dateLabel}</div>
        ${rows}
      </div>`;
  }).join("");
...
function showOCROverlay() {
  let el = document.getElementById("_ocrOverlay");
  if (!el) {
    ...
    el.innerHTML = `
      <div style="color:#14b8a6;font-size:11px;letter-spacing:0.25em;text-transform:uppercase;margin-bottom:20px" id="_ocrStatus">[ SCANNING IMAGE ]</div>
      <div style="width:220px;height:2px;background:#222;border:1px solid #333">
        <div id="_ocrBar" style="width:0%;height:100%;background:#14b8a6;transition:width 0.25s"></div>
      </div>
      <div style="color:#555;font-size:10px;letter-spacing:0.15em;margin-top:14px" id="_ocrSub">Initializing OCR engine...</div>
      <div style="color:#333;font-size:9px;letter-spacing:0.1em;margin-top:30px">All processing happens on your device</div>`;
    document.body.appendChild(el);
  }
  el.hidden = false;
}

function updateOCRProgress(pct, status, sub) {
  const bar = document.getElementById("_ocrBar");
  const st  = document.getElementById("_ocrStatus");
  const sb  = document.getElementById("_ocrSub");
...
function hideOCROverlay() {
  const el = document.getElementById("_ocrOverlay");
  if (el) el.hidden = true;
}

async function runOCR(imageFile) {
  if (!window.Tesseract) {
    showToast("OCR engine loading — try again in a moment.");
    return null;
  }
  showOCROverlay();
  try {
    const result = await Tesseract.recognize(imageFile, "eng", {
      logger: m => {
        if (m.status === "loading tesseract core")
          updateOCRProgress(10, "[ LOADING ENGINE ]", "Downloading OCR core...");
        else if (m.status === "initializing tesseract")
          updateOCRProgress(25, "[ INITIALIZING ]", "Setting up...");
        else if (m.status === "loading language traineddata")
          updateOCRProgress(40, "[ LOADING LANGUAGE ]", "Loading English model...");
        else if (m.status === "initializing api")
          updateOCRProgress(55, "[ STARTING ]", "Almost ready...");
        else if (m.status === "recognizing text")
          updateOCRProgress(55 + m.progress * 45, "[ READING TEXT ]",
...
function importBankScreenshot(source) {
  const el = document.getElementById("img-" + source);
  if (el) { el.value = ""; el.click(); }
}

async function handleBankScreenshot(source, input) {
  const file = input.files[0];
  if (!file) return;
  input.value = "";
  ...
  const parsed = parseBankOCRText(text, source);
  if (!parsed.length) {
    showToast("No transactions found in that screenshot. Try a cleaner, cropped image.");
    return;
  }
  ...
  const newTxns = parsed.filter(t => !existing.has(`${t.date}|${t.description}|${t.amount}`));
  state.bankTxns.push(...newTxns);
  state.bankTxns.sort((a, b) => (b.date || "").localeCompare(a.date || ""));
  saveState();
  ...
  const label = BANK_LABELS[source] || source;
  if (newTxns.length === 0) {
    showToast(`${label}: All ${parsed.length} found transactions already imported.`);
  } else {
    showToast(`${label}: Scanned ${newTxns.length} transaction${newTxns.length !== 1 ? "s" : ""} from screenshot!`);
  }
...
function parseBankOCRText(text, source) {
  const results = [];
  ...
  // Dollar amount: matches $25.99  -$25.99  +$1,200.00  -25.99  +50.00
  const amtRx = /([+\-]?\$[\d,]+\.?\d*|[+\-][\d,]+\.\d{2})/g;
  ...
  const dateRxs = [
    ...
    /\b(Mon|Tue|Wed|Thu|Fri|Sat|Sun)[a-z]*,?\s+\w+\s+\d{1,2}\b/i
  ];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Use the last dollar amount on the line
    const lastAmt = amtMatches[amtMatches.length - 1][0];
    const rawAmt  = parseFloat(lastAmt.replace(/[$,]/g, "")) || 0;
    if (Math.abs(rawAmt) < 0.01) continue;

    const isDeposit = rawAmt > 0 || lastAmt.startsWith("+");

    // Find date — check current line then neighbors
    let foundDate = null;
    const ctx = [line, lines[i - 1] || "", lines[i + 1] || ""];
    for (const c of ctx) { foundDate = extractDate(c); if (foundDate) break; }
    if (!foundDate) foundDate = today();

    // Build description: strip amounts and dates from the line
    let desc = line;
    desc = desc.replace(amtRx, "");
    for (const rx of dateRxs) desc = desc.replace(rx, "");
    desc = desc.replace(/\s{2,}/g, " ").replace(/^[\s\-_|•·:]+|[\s\-_|•·:]+$/g, "").trim();

    // If empty, grab adjacent non-amount line
    if (desc.length < 2) {
      if (i > 0 && ![...lines[i - 1].matchAll(amtRx)].length) desc = lines[i - 1].trim();
      else if (i < lines.length - 1 && ![...lines[i + 1].matchAll(amtRx)].length) desc = lines[i + 1].trim();
    }
...
// ---- Credit report screenshot import ----
function importCreditScreenshot() {
  const el = document.getElementById("img-creditreport");
  if (el) { el.value = ""; el.click(); }
}

async function handleCreditScreenshot(input) {
  const file = input.files[0];
  if (!file) return;
  input.value = "";
  ...
  if (extracted.length > 0) {
    ...
    const newItems = extracted.filter(i => i.creditor && !existing.has(i.creditor.toLowerCase()));
    if (newItems.length > 0) {
      if (confirm(`Scanned ${newItems.length} new account${newItems.length !== 1 ? "s" : ""}. Add to BANDGANG?`)) {
        newItems.forEach(i => { i.id = uid(); state.bandgang.items.push(i); });
        saveState();
        renderBandgang();
        showToast(`Added ${newItems.length} item${newItems.length !== 1 ? "s" : ""} from screenshot!`);
      }
    } else {
      showToast("Those accounts are already tracked in BANDGANG.");
    }
  } else {
    showToast("Screenshot scanned. No accounts auto-detected — you can add items manually below.");
  }
}
...
/* ===================== TXN IMPORT MODAL ===================== */
function txnImportModal(type) {
  const isScreenshot = type === "screenshot";
  const fn = isScreenshot ? "importBankScreenshot" : "importBankCSV";
  const title = isScreenshot ? "[ SCAN SCREENSHOT ]" : "[ UPLOAD CSV ]";
  const sub = isScreenshot
    ? "Pick which bank the screenshot is from:"
    : "Pick which bank the CSV is from:";

  document.getElementById("modalTitle").textContent = title;
  document.getElementById("modalBody").innerHTML = `
    <div style="font-size:11px;color:var(--text-dim);letter-spacing:0.1em;margin-bottom:14px">${sub}</div>
    <div style="display:flex;flex-direction:column;gap:8px">
      <button onclick="closeModal();${fn}('chime')"
        style="background:rgba(30,198,119,0.08);border:1px solid #1ec677;color:#1ec677;
               padding:13px 14px;font-family:monospace;font-size:11px;letter-spacing:0.18em;
               cursor:pointer;text-align:left;text-transform:uppercase">
        &#9654; Chime
      </button>
      <button onclick="closeModal();${fn}('cashapp')"
        style="background:rgba(0,197,50,0.08);border:1px solid #00c532;color:#00c532;
               padding:13px 14px;font-family:monospace;font-size:11px;letter-spacing:0.18em;
               cursor:pointer;text-align:left;text-transform:uppercase">
        &#9654; Cash App
      </button>
      <button onclick="closeModal();${fn}('navyfed')"
        style="background:rgba(91,141,217,0.08);border:1px solid #5b8dd9;color:#5b8dd9;
               padding:13px 14px;font-family:monospace;font-size:11px;letter-spacing:0.18em;
               cursor:pointer;text-align:left;text-transform:uppercase">
        &#9654; Navy Federal
      </button>
